...
public class TestJEEDataFlowNodeLifeCycleControl implements DataFlowNodeLifeCycleControl
{
    private static final Logger logger = Logger.getLogger(TestJEEDataFlowNodeLifeCycleControl.class.getName());
                ...
                if (field.isAnnotationPresent(DataConsumerInjection.class))
                {
                    try
                    {
                        ...
                        if (field.getType().isAssignableFrom(DataConsumer.class))
                            field.set(dataFlowNode, new DummyObserverDataConsumer<>(dataFlowNode, dataConsumerInjection.methodName(), (Class<?>) ((ParameterizedType) field.getGenericType()).getActualTypeArguments()[0]));
                        else if (field.getType().isAssignableFrom(ReferrerDataConsumer.class))
//                           field.set(dataFlowNode, null); // TODO: Implement DummyReferrerDataConsumer
                           throw new UnsupportedOperationException("ReferrerDataConsumer not yet supported");
                ...
                if (field.isAnnotationPresent(DataProviderInjection.class))
                {
                    try
                    {
                        ...
                        if (field.getType().isAssignableFrom(DataProvider.class))
                            field.set(dataFlowNode, new DummyObservableDataProvider<>(dataFlowNode));
                        else if (field.getType().isAssignableFrom(NamedDataProvider.class))
//                            field.set(dataFlowNode, null); TODO: Implement DummyReferrerDataProvider
                            throw new UnsupportedOperationException("NamedDataProvider not yet supported");
                ...
                if (field.isAnnotationPresent(DataFlowNodeStateInjection.class))
                {
                    try
                    {
                        ...
                        {
                            DataFlowNodeState dataFlowNodeState = new DummyDataFlowNodeState(dataFlowNodeId);
